function checkLoginStatus(){
    var loggedInContent = document.getElementsByClassName("loggedInShow");
    var loggedOutContent = document.getElementsByClassName("loggedOutShow");
...
async function createAccount(){
    let uName = document.getElementById("nameRegister").value;
    ...
    if(fUser.data.success){
        ...
        if(!(new RegExp('^(?=.*[^a-zA-Z\s:])(?=.*[a-zA-Z]).{8,}$').test(pass))){ //mix from https://regexr.com/3bfsi and online regex resources
        ...
        if(passVerif){
            let postedUser = await axios.post("/createUser", {
                uname: uName,
                passw: pass
              });
...
async function logIntoAccount(){
    let uName = document.getElementById("nameLogin").value;
